...
while (1) { // C언어와 같이 0이 아닌 모든 값을 truthy값으로 평가.
    let value = prompt("숫자를 입력하세요.");

    if (value == 0) break; // == (equal) : 피연산자들의 값만 비교. 단, 숫자형태의 문자열은 다른 숫자 타입의 값과
                        // 비교시 숫자 타입으로 형변환되어 비교. 따라서 입력된 문자열 '0'과 '확인' 버튼을 눌렀을 때 
                        // 반환되는 빈 문자열 ('')은 모두 숫자 0으로 변환되어 비교.
    sum += value;       // 단,원시값 중 빈 문자열('')과 false 만 추상 동등 비교(==) 시 0 으로 변환되어 평가.
