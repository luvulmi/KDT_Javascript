...
//         if (itemData[0][i] === null) return 1;      // 취소 입력 시 true.
...
//         return 0; // 함수의 종료 역할. 해당 없으면 false로 탈출. break 사용해도 무방.
//     }
// }
...
function checkInput(i) {
    ...
    if (itemData[0][i] === null) return 1;
        ...
        return checkInput(i); // 재귀호출을 적용함으로써 자기자신을 계속 호출. -> 반복문 사용할 필요 x 
    }
    return 0;
}
...
while (1) {
    for (let i = 0; i < itemData[0].length; i++) {
        if (checkInput(i)) break outer; // 취소 입력 시 true -> 탈출.
    }
